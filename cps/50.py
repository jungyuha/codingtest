...
territory = []
territory.append([0 for i in range(w)])
for i in range(0,h):
    territory.append(list(map(int,sys.stdin.readline().split())))
for i in range(0,h):
    territory[i].insert(0,0)
#print(territory)
hh , ww = map(int,sys.stdin.readline().split())
max = -9999
#for i in range(h):
#    print(territory[i])
for i in range(1,w-ww+1): # 가로
    for j in range(1,h-hh+2): # 세로
        #print("i , j : %d , %d" %(i , j))
        sum = 0
        for s in range(j,j+hh):
            for k in range(i,i+ww):
                sum = sum + territory[s][k]
                #print("territory[k][s] : %d " % (territory[k][s]))
        #print("sum : %d" % (sum))
        if (sum > max) :
            max = sum
print(max)