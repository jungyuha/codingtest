...
# n : 전체 왕자의 수
# bp : 탈락한 왕자 수
# cnt : 왕자들이 부르고 있는 숫자 현황
# p : 현재 숫자를 부르고 있는 왕자의 위치
prince = [0 for _ in range (0,n+1)]
...
#    print("p , n : %d %d" %(p,n))
    ...
    if(prince[p] == 0):
        cnt = cnt + 1
        if(cnt == k):
            #print("cnt , k : %d %d" % (cnt , k))
            prince[p] = 1   # 탈락하면 1 (즉 최종으로 남은 왕자는 0이다.)
            cnt = 0
            bp = bp +1
            #print("cnt , k , bp : %d %d %d " % (cnt, k,bp))
            #print(prince)
    if(bp == n-1):
        break
#print(prince)
