...
bool check(int cy , int cx , int k , int hole_){
    ...
    if(min_val < hole_ && max_val > hole_) { 
        ...
        //cout <<", res : "<<res<<"     ";
        if(end_yn) {val_[cy][cx] = hole_; q1.push({cy,cx});} // val_[cy][cx] = hole_; 
        else if(val_[cy][cx] >= val_[ny][nx]) { val_[cy][cx] = max(res,hole_); val_[ny][nx] = min(res,hole_); q1.push({cy,cx}); q1.push({ny,nx});}
        else if(val_[cy][cx] < val_[ny][nx]) { val_[cy][cx] = min(res,hole_); val_[ny][nx] = max(res,hole_); q1.push({cy,cx}); q1.push({ny,nx});}
         return true; }
    else { return false; }
}
int main(){
